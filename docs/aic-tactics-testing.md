...
Use the signed secure [framework preview](https://github.com/UnofficialCrusaderPatch/UnofficialCrusaderPatch3/actions/runs/34718954887), then copy this bundle's `ucp/modules` contents into your game's `ucp/modules`, keeping every module ZIP zipped and its `.zip.sig` beside it; enable AIC Tactics in the UCP GUI and apply `AIC-TACTICS-COMPATIBILITY.md`.
...
Choose framework artifact `UCP3-3.0.7-abf60343d5` (secure). This build uses the shared scanner correction in [framework PR149](https://github.com/UnofficialCrusaderPatch/UnofficialCrusaderPatch3/pull/149); stock 3.0.7 failed the startup performance gate. The final supported framework release is still pending.
